...
int main(void) {
    printf("Day 1\n");
    int n_elems = read_input_lists(input_file_name, MAX_LIST_LENGTH, list1, list2);
    if (n_elems < 0) {
        return -1;
    }
...
int part2(const int n_elems, const int *list1, const int *list2) {
    ...
    for (int list1_idx=0; list1_idx < n_elems; ++list1_idx) {
        ...
        while (list2_idx < n_elems && list2[list2_idx] < cur_elem) {++list2_idx;}
        int count = 0;
        while ((list2_idx+count) < n_elems && list2[list2_idx+count] == cur_elem) {++count;}
